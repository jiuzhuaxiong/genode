...
int connection_exists(Genode::addr_t phonehash)
{
...
int delete_connection(Genode::addr_t phonehash)
{
	...
	if((pos = connection_exists(phonehash)) >= 0) {
			_callid[pos] = 0;
			_phone_hash[pos] = 0;
			_task_id[pos] = 0;
			_call_counter--;
		...
		Genode::addr_t new_phonehash, Genode::Native_task new_task_id)
{
	...
	if(retval == EOK) /* connection is established succefully */
		retval = insert_new_connection(new_callid, new_phonehash, new_task_id);
...
Genode::addr_t reject_connection(Genode::Native_ipc_callid callid)
{
	...
	return Spartan::ipc_answer_0(callid, 0);
}
...
//		callid = Spartan::ipc_trywait_for_call(&call);
		...
		switch(IPC_GET_IMETHOD(call)) {
			case IPC_M_CONNECT_TO_ME:
				Genode::printf("nameserv:\treceived connection request with "
					" callid = %lu,\n\t   in_task_id = %lu, "
					"in_phone_hash = %lu\n", callid,
					call.in_task_id, call.in_phone_hash);
				retval = accept_connection(callid, call.in_phone_hash, call.in_task_id);
				if(retval == EOK) 
					Genode::printf("nameserv:\tconnection established to task %lu\n", call.in_task_id);
				else
					Genode::printf("nameserv:\tcould not establish "
						"connection. Errorcode %lu\n", retval);
				break;
			default:
				retval = reject_connection(callid);
				Genode::printf("nameserv:\tconnection rejected with "
					"returncode %lu\n", retval);
		}
	}
